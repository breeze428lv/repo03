...
void SendStringsWithSplitter(uchar *bytes, uint8 length)
{
	uint8 i;
	char buff[5];
	char* p;
...
*/
/*
void SetPowerSaving(uint8 enable, uint8 bl_on_level, uint8 bl_off_level, uint8  bl_on_time)
{
    BEGIN_CMD();
    TX_8(0x77);
    TX_8(enable);
...
void SetPowerSaving(uint8 enable, uint8 bl_on_level, uint8 bl_off_level, uint16  bl_on_time)
{
    BEGIN_CMD();
    TX_8(0x77);
    TX_8(enable);
...
void Record_Add_Byte(uint16 screen_id,uint16 control_id,uint8 *bytes, uint8 length)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x52);
    ...
    SendStringsWithSplitter(bytes, length);
...
void ShowIcon(uint16 screen_id,uint16 control_id,uint8 frame_index)
{
